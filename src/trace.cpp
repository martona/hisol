...
void log_http_response(const http::response<http::string_body>& response, std::string_view decoded_body, bool enabled)
{
    ...
    std::cerr << "hisol: <<< HTTP/1."
              << response.version() % 10 << ' '
              << response.result_int() << ' '
              << response.reason() << '\n';
    log_headers(response.base(), "hisol: <<< ");
...
void log_websocket_request(const Url& url, const CookieJar& cookies, bool enabled)
{
    ...
    std::cerr << "hisol: >>> GET /sol HTTP/1.1\n";
    std::cerr << "hisol: >>> Host: " << make_host_header(url) << '\n';
    std::cerr << "hisol: >>> User-Agent: hisol/" << BOOST_LIB_VERSION << '\n';
    std::cerr << "hisol: >>> Upgrade: websocket\n";
    std::cerr << "hisol: >>> Connection: upgrade\n";
    std::cerr << "hisol: >>> Sec-WebSocket-Key: " << kRedacted << '\n';
    std::cerr << "hisol: >>> Sec-WebSocket-Version: 13\n";
    std::cerr << "hisol: >>> Origin: " << make_origin(url) << '\n';
    if (!cookies.header().empty()) {
        std::cerr << "hisol: >>> Cookie: " << kRedacted << '\n';
    }
}
